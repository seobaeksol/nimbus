...
export class PathAliasService {
  ...
  static async initialize(): Promise<void> {
    try {
      // Import FileService dynamically to avoid circular imports
      const { FileService } = await import('./fileService');
      const backendPaths = await FileService.getSystemPaths();
      ...
      console.error('Failed to initialize system paths from backend, falling back to detection:', error);
  ...
  static async resolvePath(input: string): Promise<string> {
    ...
    try {
      ...
      const { FileService } = await import('./fileService');
      const resolvedPath = await FileService.resolvePath(input);
      return resolvedPath;
    } catch (error) {
   ...
   */
  static getAvailableAliases(): { alias: string; path: string; description: string }[] {
    const aliases: { alias: string; path: string; description: string }[] = [];

    // System aliases
    aliases.push(
      { alias: '~', path: this.systemPaths?.home || '/', description: 'Home directory' },
      { alias: '/', path: '/', description: 'Root directory' },
      { alias: 'Documents', path: this.systemPaths?.documents || '~/Documents', description: 'Documents folder' },
      { alias: 'Downloads', path: this.systemPaths?.downloads || '~/Downloads', description: 'Downloads folder' },
      { alias: 'Desktop', path: this.systemPaths?.desktop || '~/Desktop', description: 'Desktop folder' },
      { alias: 'Music', path: this.systemPaths?.music || '~/Music', description: 'Music folder' },
      { alias: 'Pictures', path: this.systemPaths?.pictures || '~/Pictures', description: 'Pictures folder' },
      { alias: 'Videos', path: this.systemPaths?.videos || '~/Videos', description: 'Videos folder' }
    );
    ...
    this.userAliases.forEach((userAlias) => {
      aliases.push({
        ...
        path: userAlias.path,
        description: userAlias.description || `Custom alias for ${userAlias.path}`
      });
    });
